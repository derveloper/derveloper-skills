...
def _briefing_gate_prompts(*, wt_path: Path, base: str) -> str:
    ...
    Subagents are routed by type to scoped plugin agents:
      - GATE 2: tmux-pair:gate-2-plan-check (Sonnet, Read+Grep+Glob+Bash; no
        Edit/Write tools, so it cannot accidentally commit code)
      - GATE 3: tmux-pair:gate-3-verifier (Haiku, Read+Grep+Glob+Bash; runs
        builds/tests, checks plan coverage)
      - GATE 3: tmux-pair:gate-3-code-reviewer (Sonnet, Read+Grep+Glob+Bash;
...
def _briefing_orchestrator(
    *, writer_pane: str, writer_agent: str,
    ...
    return (
        ...
        f"Du führst Writer + Reviewer durch einen 4-Gate-Workflow:\n"
        ...
        f"4. GATE 2: PLAN-CHECK (Subagent, scoped)\n"
        ...
        f"7. GATE 3: FINAL-VERIFY (Subagents scoped, PARALLEL spawnen)\n"
