...
- `--pi-<role>-model <slug>` / `--pi-<role>-thinking <level>` — pro-Rolle Override für pi-Panes. Rollen: `writer`, `reviewer`, `orchestrator`, `reviewer-2` (mit `--dual-review`). Wenn gesetzt, gewinnt die Rolle-Variante gegenüber `--pi-model` / `--pi-thinking`. Beispiel: `--writer-agent pi --pi-writer-model glm-5.1 --reviewer-2-agent pi --pi-reviewer-2-model kimi-k2.6 --dual-review` mischt zwei pi-Modelle im selben Spawn.
