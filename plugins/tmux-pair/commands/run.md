...
argument-hint: <project-path> <base> <feature> [task...] [--solo|--spawn] [--size 3|4|5] [--parallel-writers] [<any mode-specific flag>]
---
...
- `--spawn`: skip recommendation, invoke `/spawn` directly. Pair this with `--size` and `--parallel-writers` if you already know the layout.
...
- `/run ~/code/myapp main parallel-rewrite split storage module into 3 backends --spawn --size 4 --parallel-writers` (force spawn-4-parallel-writers)
...
3. **Mode recommendation**:
   ...
   - **spawn --size 4 --parallel-writers** when: the task has obvious parallel-friendly sub-tasks with disjoint files (e.g. split a module into N backends; bulk migration across independent areas). Two writers on disjoint bullets.
   - **spawn --size 5** when: both dual-review and parallel-writers are warranted (big feature on security-sensitive code).
...
- `--spawn`: invoke `/spawn` directly, skip recommendation. All `/spawn` flags (including `--size`, `--parallel-writers`, `--writer-agent`, etc.) are forwarded.
...
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/tmux_pair.py spawn \
  --project <project-path> --base <base> --feature <feature> --task "<task>" \
  --size <recommended> [--parallel-writers] \
  [...forwarded flags]
```
