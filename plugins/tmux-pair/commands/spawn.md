---
description: Spawn a coordinated agent team (orchestrator + ONE writer + reviewers, size 3..4) in a fresh git worktree, with PROJECT.md care in the gated workflow. Parallel work via subagent-worktrees.
argument-hint: <project-path> <base> <feature> [task...] [--size 3|4] [--interactive] [--with-standards] [--greenfield] [--no-worktree] [--writer-agent NAME] [--reviewer-agent NAME] [--reviewer-2-agent NAME] [--orchestrator-agent NAME] [--claude-model SLUG] [--claude-effort LEVEL] [--codex-effort LEVEL] [--reviewer-claude-effort LEVEL] [--reviewer-codex-effort LEVEL] [--pi-model SLUG] [--pi-thinking LEVEL]
---

# spawn

Spawn a coordinated team of agents in a fresh `git worktree`. Team size is set via `--size` (3 or 4, default 3). The orchestrator runs in a pane on top across the full width; the single writer + reviewer(s) sit beneath. The orchestrator does recon, writes the engineer briefings, watches the implementation loop, and reports up to the human pane on major events only (`COMPLETE` after GATE-3-PASS, `ABORT` if irreparable).
...
|----------|---------|-----------|--------------|----------|
| 3 (default) | 1 | 1 | 1 | standard task, one writer + one reviewer is enough |
| 4 | 1 | 2 | 1 | risky / security-sensitive: two reviewers cross-check, orchestrator consolidates |

There is no second writer pane. Parallel work happens via subagent-worktrees the writer spawns from its Task tool (see below).

Every `send` message gets a stable sender prefix such as `[FROM: or.<feature>]`, `[FROM: wr.<feature>]`, or `[FROM: rv.<feature>]` (or `rv1`/`rv2` when dual-review), unless the message already starts with `[FROM:`. The helper stores sender names in tmux pane options so agent TUI spinner titles do not leak into pings.
...
Plans must include explicit parallel markers per bullet: `B3 || B4 [parallel]` when work can run together, or `B3 -> B4 [sequenziell: <reason>]` when ordering is required. The writer fans parallel bullets out via subagent-worktrees; sequential bullets stay in the main writer pane.

## Subagent-worktree pattern (parallel work)

When the plan contains `B<x> || B<y> [parallel]` markers, the writer:

1. Creates a sub-worktree per parallel bullet:
   `git worktree add ../<feature>-sub-<bullet-id> -b <feature>/sub-<bullet-id>`
2. Spawns one `Task(general-purpose)` subagent per sub-worktree, which works there with isolated files.
3. After subagent-DONE: fast-forward merge back into the feature-WT:
   `git -C <feature-wt> merge --ff-only <feature>/sub-<bullet-id>`
   FF failure means the feature-WT moved on. Writer pings `CLARIFY-NEEDED` to the orchestrator instead of forcing a merge-commit.
4. Cleanup: `git worktree remove ../<feature>-sub-<bullet-id>` + `git branch -D <feature>/sub-<bullet-id>`.
5. The final feature -> main merge is a **squash** done by the master after GATE-3-PASS, not by the writer. The feature branch keeps sub-merge history; main stays linear.

## Invocation

`/spawn <project-path> <base> <feature> [task...] [--size 3|4] [--interactive]`
...
- `--size <3|4>` (default 3): team size. 3 = 1W/1R/1O. 4 = 1W/2R/1O (dual-review preset).
...
- `--claude-effort <level>`: claude reasoning effort, set as `--effort <level>` in the claude boot-command (default `medium`, applies to non-reviewer claude panes: writer + orchestrator). Choices: `low|medium|high|xhigh|max`. Pass an empty string to skip the flag (claude default or `CLAUDE_CODE_EFFORT_LEVEL` env-var applies).
- `--codex-effort <level>`: codex reasoning effort, set as `-c model_reasoning_effort=<level>` in the codex boot-command (default `medium`, applies to non-reviewer codex panes). Choices: `minimal|low|medium|high`. Empty string skips the flag.
...
- `--writer-agent <name>` / `--reviewer-agent <name>` / `--reviewer-2-agent <name>` / `--orchestrator-agent <name>`: Agent-Wahl pro Rolle. Erlaubt: `claude` (Default Writer + Orchestrator), `codex` (Default Reviewer + Reviewer-2), `pi` (Custom CLI mit Cortecs als Default-Backend für günstige Bulk-Work).
...
- `--pi-<role>-model <slug>` / `--pi-<role>-thinking <level>` / `--pi-<role>-provider <name>`: pro-Rolle Override für pi-Panes. Rollen: `writer`, `reviewer`, `reviewer-2`, `orchestrator`. Beispiel: `--writer-agent pi --reviewer-agent pi --pi-reviewer-provider claude-bridge --pi-reviewer-model claude-opus-4-7` mischt Cortecs-Writer (cheap) mit Anthropic-Reviewer (top-gate).
...
- two reviewers consolidating reduce false-APPROVE risk (size 4)
- the plan has parallel-friendly bullets the writer can fan out via subagent-worktrees
...
JSON with `mode: "spawn"`, `size`, `writers`, `reviewers`, `dual_review`, `worktree`, `branch`, `window`, `orchestrator_pane`, `orchestrator_name`, `writer_pane`, `writer_name`, `reviewer_pane`, `reviewer_name`, `human_pane`. With dual-review active (reviewers >= 2): additional `reviewer_2_pane`, `reviewer_2_agent`, `reviewer_2_name`. Relay back to the user.
