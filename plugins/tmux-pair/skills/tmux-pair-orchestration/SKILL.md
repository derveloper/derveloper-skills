...
- **pi panes** boot with `--append-system-prompt <path>` (the users Custom-CLI, `~/.pi/agent/`). pi liest zusätzlich `AGENTS.md` und `CLAUDE.md` via Default-Discovery, also wirkt der codex-Pfad transitiv mit. Default-Model `qwen3-coder-next` via Default-Provider `cortecs` (EU Pay-per-Use, ~0.15/0.80 EUR pro 1M Tokens, 256k ctx, coder-spec), default `--thinking high`. Cortecs ist als Default gesetzt für günstige Bulk-Work; Anthropic-Subscription weiter zugänglich via `--pi-provider claude-bridge --pi-model claude-opus-4-7` (Reviewer/Orchestrator als Top-Gate). Override per Spawn via `--pi-provider`, `--pi-model`, `--pi-thinking`. Bekannte Beschränkungen: kein mid-session `/model`-Wechsel (Pane-Restart nötig) und kein `/compact`-Equivalent (Compact-Watcher pingt pi-Panes nicht).
