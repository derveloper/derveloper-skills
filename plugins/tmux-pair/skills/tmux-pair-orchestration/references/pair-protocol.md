...
| `CLARIFY-NEEDED` | Stuck on a question only the user can answer (scope, behavior, UX, architecture choice, naming conflict, trade-off not in plan) | the question + 2-4 concrete options with trade-offs. Sent to the orchestrator. The receiver translates this into an `AskUserQuestion` call; engineers do NOT decide user-facing questions on their own. |
...
## Orchestrator events (spawn mode only)
...
These extend the base events above. They drive the gated workflow described in `references/gated-workflow.md`. Events flow between orchestrator and human; the orchestrator owns user-facing decisions via its own `AskUserQuestion`.

| Event | From | To | When | Payload |
|-------|------|-----|------|---------|
| `GATE-1-ESCALATE <window>` | orchestrator | human | Rare. Orchestrator can't decide a clarify question alone (budget, scope shift, stakeholder dependency, user unreachable) | reason + question(s) needing human input |
| `GATE-1-DECISION` | human | orchestrator | After human answered the escalation | answer(s), context |
...
| `REVIEW-FINAL (Reviewer):` | reviewer | orchestrator | After both `REVIEWER-FINDINGS:` + `PEER-REVIEW:` cycles complete | Merged final findings from this reviewer's perspective + APPROVE or BLOCK verdict. |
...
5. The orchestrator forwards `[Orchestrator <window>] DONE: <sha> <branch state>` to human.

Push happens only after human OK. Whether the briefing said so explicitly or not, the orchestrator waits.
...
Writer pings orchestrator with `BLOCKER: <one-paragraph framing of the disagreement>`. Orchestrator decides: ask human via `AskUserQuestion`, or break the tie themselves if the disagreement is about style and not correctness.
...
The spawn loses its value the moment writer commits without `REVIEW: APPROVE`. The briefing should say this explicitly: "do not commit without an APPROVE event from the reviewer". If the writer skips anyway, the reviewer pings `PROCESS-NEEDS-FIX` to the writer with a copy to the orchestrator.
